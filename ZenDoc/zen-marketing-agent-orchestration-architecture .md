...
## Phase 2 Status — Responsive Wizard (Implemented)

**Implemented:**
- New wizard shell (`.magent-shell`) — genuinely progress-left/decision-right on desktop as the spec explicitly requested, collapsing to a horizontally-scrollable step nav + sticky bottom action bar on mobile. Deliberately not `wizSteps()` — that component is horizontal-only and doesn't support this layout, so building a new one was the honest choice over force-fitting an existing pattern that doesn't fit the spec
- Entry point added to the Marketing hub, styled as the recommended path, with existing shortcuts (Make a poster, Meta ad strategies, etc.) explicitly still visible and untouched beneath it
- **Step 1 (Goal):** objective, budget, customer count, location/service-area — all real inputs, real validation before continuing
- **Step 2 (Audience/Persona):** fully real, library-first as specified. `get_persona_recommendations` checks the seeded library (tag-matched against the tenant's declared industry) before generating anything; only generates the shortfall if fewer than 3 real fits exist. "See more" reveals additional real library matches. Custom persona creation is real (`save_custom_persona`, tenant-scoped, persists immediately and becomes selectable in the same session)
- **Steps 3-7:** honest placeholders — each names the exact phase that builds it and links to the two working shortcuts (`contentFlow()`, `goalStrategyFlow()`) as the way to get that outcome today. Not faked, not skipped silently

**Reused, not touched:** `goalStrategyFlow()` and `contentFlow()` — confirmed still fully reachable, both from the Marketing hub directly and from within the wizard's placeholder steps, per the explicit instruction not to delete or absorb them this phase

**Verified:**
- Full flow tested end-to-end: Goal → real persona recommendations called and rendered → selection → honest Step 3 placeholder, with every assertion passing (library personas shown correctly, hypothesis flagging correct, "See more" reveals the right item, custom persona path wired)
- Desktop layout confirmed visually: progress left, decision right, as specified
- Mobile layout confirmed both structurally (`display:block` collapse, `position:sticky` action bar) and visually via screenshot
- Existing Marketing hub shortcuts confirmed still present and unmodified

**Not yet done, correctly deferred:**
- Steps 3-7's real implementation (Phases 3-5 per the plan)
- The persona library needs its 20 seed rows actually inserted in the live database — SQL provided below, not yet run against production
- Persona "edit" (as opposed to select-as-is or create-new) isn't built this phase — a selected persona can't currently be tweaked before continuing; flagging as a real gap against the spec's "accept / edit / create their own" three-way choice, since only two of the three are genuinely built

## Migrations Required (Phase 2)

```sql
create table if not exists mw_persona_library (
  id uuid primary key default gen_random_uuid(),
  tenant_id text,                    -- null = global library entry, visible to every tenant
  name text not null,
  who_they_are text,
  business_types text[] default '{}',
  targeting_approach text,
  creative_angle text,
  is_hypothesis boolean default false,
  created_at timestamptz not null default now()
);
create index if not exists mw_persona_library_tenant_idx on mw_persona_library(tenant_id);
```

Then run the 20-row seed insert (in the attached `persona-library-seed.sql`) once against the live database.

---

## Architecture Amendment — Marketing Intelligence as a True Vertical (Salon-first)

**What changed and why:** the previous design (loose `business_types text[]` tags on generic entries) treats every industry as a shallow variant of the same shape. A genuine marketing intelligence engine needs the opposite: *deep*, industry-specific knowledge for one vertical done properly, with the architecture built so a second and third vertical are a real extension, not a rewrite. Salon is the proven vertical (real tenant, real data, real history in this codebase) and becomes the first fully-built one. Restaurant, Clinic, etc. get registered as real rows with **no content yet** — an honest "not built" state, not a shallow generic filler pretending to be vertical-specific.

### Schema: vertical registry replaces loose tags

```sql
create table if not exists mw_marketing_verticals (
  id uuid primary key default gen_random_uuid(),
  key text not null unique,              -- 'salon', 'restaurant', 'clinic', ...
  name text not null,
  status text not null default 'planned' check (status in ('planned','active')),
  created_at timestamptz not null default now()
);
-- Seed: salon is 'active' (real content follows). Restaurant, Clinic,
-- Retail inserted as 'planned' - registered, empty, honestly labeled.
insert into mw_marketing_verticals (key, name, status) values
  ('salon', 'Salon & Personal Care', 'active'),
  ('restaurant', 'Restaurant & Food Service', 'planned'),
  ('clinic', 'Clinic & Wellness', 'planned'),
  ('retail', 'Retail', 'planned')
on conflict (key) do nothing;
```

**`mw_marketing_playbooks` restructured** — vertical-keyed, not tag-matched:

```sql
create table if not exists mw_marketing_playbooks (
  id uuid primary key default gen_random_uuid(),
  vertical_id uuid not null references mw_marketing_verticals(id),
  category text not null,                -- 'retargeting','device_targeting','creative_testing','seasonal', etc.
  tactic_name text not null,
  when_to_use text not null,
  how_to_execute text not null,
  expected_signals text,                 -- what real data would confirm this worked
  is_hypothesis_framework boolean default true,  -- true = this describes a testable approach, not a proven fact
  created_at timestamptz not null default now()
);
create index if not exists mw_marketing_playbooks_vertical_idx on mw_marketing_playbooks(vertical_id, category);
```

**`mw_persona_library` gets a `vertical_id`, and the existing 20 get reclassified honestly, not silently kept as-is:**

```sql
alter table mw_persona_library add column if not exists vertical_id uuid references mw_marketing_verticals(id);
-- The 20 personas already seeded stay as-is but get explicitly marked
-- cross-vertical starter content (vertical_id null = applies broadly,
-- same as before) - they are NOT rewritten as fake salon-specific
-- depth they don't actually have. Real salon depth comes from new,
-- additional personas below, not from relabeling the generic ones.
```

**New, genuinely salon-specific personas** (additive to the 20, not replacing them) — this is where real vertical depth lives:

```sql
insert into mw_persona_library (vertical_id, tenant_id, name, who_they_are, targeting_approach, creative_angle, is_hypothesis) values
((select id from mw_marketing_verticals where key='salon'), null, 'First Haircut Milestone Parent', 'Parents of a toddler about to get their very first haircut - an emotionally significant, one-time moment, not a routine service.', 'Interest-based: new parents, toddler age-range households; broad radius', 'The milestone itself is the hook - "their first haircut, done right", keepsake/certificate framing', false),
((select id from mw_marketing_verticals where key='salon'), null, 'Multi-Sibling Coordinator', 'Manages visits for 2+ children with different needs/ages at once - values being able to book and handle the whole family in one trip.', 'Broad, parents 28-45, family-size-aware messaging', 'Whole-family, single-trip convenience - not a per-child pitch', false),
((select id from mw_marketing_verticals where key='salon'), null, 'Special Needs Accommodation Seeker', 'Actively searching for a salon experienced and patient with sensory-sensitive or special-needs children - a real, specific, underserved need.', 'Interest-based where Meta supports it, broad otherwise - creative-led targeting more than demographic', 'Patience, experience, and specific accommodation - substantiated claims only, never invented credentials', false),
((select id from mw_marketing_verticals where key='salon'), null, 'Photo-Day Prep Parent', 'Booking specifically ahead of school photo day or a similar dated event - urgency is real and specific, not manufactured.', 'Time-bound, broad targeting in the real run-up window before the actual known date', 'Tied to the real, specific date - "before picture day", not a generic seasonal push', false);
```

**Salon-specific playbook entries** (real tactical depth, not generic advice):

```sql
insert into mw_marketing_playbooks (vertical_id, category, tactic_name, when_to_use, how_to_execute, expected_signals, is_hypothesis_framework) values
((select id from mw_marketing_verticals where key='salon'), 'retargeting', 'Low-frequency nurture retargeting', 'Customer has visited 1-2 times total, most recent visit still recent - not lapsed, not yet a regular.', 'Custom Audience filtered to visit_count between 1 and 2, most recent visit within 60 days; message emphasizes building the relationship, not a cold offer.', 'Third-visit conversion rate for this audience vs. baseline first-to-second-visit rate', true),
((select id from mw_marketing_verticals where key='salon'), 'device_targeting', 'Premium device/OS as a spend-value hypothesis', 'Testing whether iOS/newer-device users produce higher average transaction value for this specific salon.', 'Split-test two ad sets, identical creative and budget, one constrained to iOS, control unconstrained - never assume the result, always run the test.', 'Compare CAC AND average transaction value together, not CAC alone - a higher-CAC segment can still be the better one if spend-per-visit is high enough', true),
((select id from mw_marketing_verticals where key='salon'), 'seasonal', 'Back-to-school haircut window', 'The 2-3 weeks before a known school start date - genuine, dated urgency, not a manufactured one.', 'Time-bound campaign tightly bounded to the real local school calendar, not a generic "seasonal" date range.', 'Booking volume in the specific window vs. the same weeks in a non-back-to-school period', true),
((select id from mw_marketing_verticals where key='salon'), 'creative_testing', 'Milestone vs. convenience angle test', 'Testing whether emotional/milestone framing (first haircut, special occasion) outperforms convenience framing (quick, reliable) for the same audience.', 'Two creative concepts, same targeting, same budget - genuinely different angle, not the same copy reworded.', 'CTR alone is not enough - compare through to actual booking/visit rate, since milestone framing may get fewer clicks but better-qualified ones', true);
```

### Cross-tenant evidence aggregation — a real consideration, deliberately not resolved silently

`mw_tenant_evidence` (from the earlier engine design) now carries a `vertical_id`. The genuinely powerful long-term capability is: once several salon tenants have each independently confirmed "low-frequency nurture retargeting works," that pattern should strengthen faster for a *new* salon tenant than starting from zero. **This requires deliberate anonymization and an explicit opt-in decision before any cross-tenant aggregation happens** — pooling evidence across businesses without consent is a real privacy line, not a technical detail to wave through. This document flags it as a required decision, not something the architecture should quietly assume either way. Until that decision is made, evidence stays strictly per-tenant, same as designed originally.

### Updated implementation phase plan

The intelligence layer is real, foundational work that Creative Strategy (originally Phase 3) should be built on top of, not alongside — inserting it before Phase 3 rather than folding it in.

| Phase | Content | Status |
|---|---|---|
| 1 | Unification/Foundation | ✅ Shipped |
| 2 | Responsive Wizard (Goal, Audience/Persona) | ✅ Shipped |
| **2.5 (NEW)** | **Marketing Intelligence Vertical — Salon** | Not started — this document is the design |
| 3 | Creative Strategy / Format Mix, Replicate wiring | Not started — now depends on 2.5's playbook structure existing first |
| 4 | Publishing | Not started |
| 5 | Meta Execution (incl. the objective-mapping fix flagged earlier) | Not started |
| 6 | Organic-to-Paid | Not started |
| 7 | Attribution & Learning (now also writes into `mw_tenant_evidence`) | Not started |
| 8 | Autopilot | Not started |
| 9 | QA / Gap Audit | Not started |

**Phase 2.5 scope, concretely:**
1. `mw_marketing_verticals` table + seed (salon active, others planned)
2. `mw_marketing_playbooks` table + real salon-specific seed content (4 tactics above, expandable)
3. `mw_persona_library` gets `vertical_id`; 4 new genuinely salon-specific personas added (above), existing 20 explicitly kept as cross-vertical starter content, not relabeled
4. `mw_tenant_evidence` table, vertical-aware, strictly per-tenant until the cross-tenant decision above is made
5. `get_persona_recommendations` extended to check vertical-specific personas first, before the cross-vertical library, before generation — a genuine three-tier fallback, not two
6. The new weekly review job (Phase 7's territory originally, but its *existence* as a scheduled action belongs here structurally, even if its full attribution depth waits for Phase 7)

**Acceptance criteria:** a salon tenant's persona recommendations visibly differ in specificity from what a hypothetical restaurant tenant would see (even though restaurant has no content yet — the point is the *mechanism* correctly returns "no vertical-specific personas yet" rather than silently falling back to something that looks vertical-specific but isn't).
